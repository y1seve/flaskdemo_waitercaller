...
class MockDBHelper:
    ...
    def get_table_number(self, _id):
        for table in MOCK_TABLES:
            if table.get('_id') == _id:
                return table.get('number')
    
    def add_request(self, table_id, time):
        table_number = self.get_table_number(table_id)
        if table_number:
            MOCK_REQUESTS.append({'_id': table_id, 'table_number': table_number, 'table_id': table_id, 'time': time})
